...
public class AdvancedClimberPathNavigator<T extends AbstractClimberEntity> extends AdvancedGroundPathNavigator<T> {
	...
	private boolean isNextTargetInLine(Vec3d pos, int sizeX, int sizeY, int sizeZ) {
		if(this.currentPath.getCurrentPathIndex() + 1 >= this.currentPath.getCurrentPathLength()) {
			...
			Vec3d currentTarget = this.currentPath.getCurrentPos().add(0.5D, 0.5D, 0.5D);
	...
	protected boolean isDirectPathBetweenPoints(Vec3d start, Vec3d end, int sizeX, int sizeY, int sizeZ) {
		switch(this.verticalFacing.getAxis()) {
		case X:
			return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, EnumFacing.Axis.Z, EnumFacing.Axis.X, EnumFacing.Axis.Y, 0.0D, this.verticalFacing.getXOffset() < 0);
		case Y:
			return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, EnumFacing.Axis.X, EnumFacing.Axis.Y, EnumFacing.Axis.Z, 0.0D, this.verticalFacing.getYOffset() < 0);
		case Z:
			return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, EnumFacing.Axis.Y, EnumFacing.Axis.Z, EnumFacing.Axis.X, 0.0D, this.verticalFacing.getZOffset() < 0);
		}
		return false;
	}

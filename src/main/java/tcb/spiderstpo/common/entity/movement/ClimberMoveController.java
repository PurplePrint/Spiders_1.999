...
public class ClimberMoveController extends EntityMoveHelper {
	...
	public void onUpdateMoveHelper() {
		...
		if(this.action == EntityMoveHelper.Action.MOVE_TO) {
			...
			double dx = (this.posX + Math.max(0, side.getXOffset()) * (entitySizeX - 1) + side.getXOffset() * 0.5f) - this.entity.posX;
			double dy = (this.posY + Math.max(0, side.getYOffset()) * (entitySizeY - 1) + side.getYOffset() * 0.5f) - this.entity.posY;
			double dz = (this.posZ + Math.max(0, side.getZOffset()) * (entitySizeZ - 1) + side.getZOffset() * 0.5f) - this.entity.posZ;
